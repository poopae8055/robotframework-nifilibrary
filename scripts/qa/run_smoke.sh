...
echo $COMPONENT
RESULT_FOLDER="results/"$ENV"_smoke"
...
date
if command -v python3 &>/dev/null; then
    python3 -m robot.run -L TRACE -v env:$ENV -e not_ready -e not_active -e not_test -i smoke_test ../../testcases/
else
    python -m robot.run -L TRACE -v env:$ENV -e not_ready -e not_active -e not_test -i smoke_test ../../testcases/
fi
date
